...
function App() {
  const [role, setRole] = useState('patient'); // 'patient' | 'nurse' | 'doctor'
  ...
  const [patientForm, setPatientForm] = useState({
    name: '', age: '', gender: 'Male', device_id: '', travel_history: '', past_medical_history: ''
  });
  ...
  const handleLogin = async (e) => {
    ...
    if (role === 'patient') {
      try {
        ...
        const found = data.find(p => p.device_id.toLowerCase() === loginForm.username.trim().toLowerCase());
        if (found) {
          ...
          showStatus("No patient profile found with this Device ID. Please register first.", "error");
        }
      } catch (err) {
        showStatus("Network error during patient login.", "error");
      }
    } else if (role === 'nurse') {
      if (loginForm.username.trim() === 'nurse' && loginForm.password === 'password') {
        setIsLoggedIn(true);
        showStatus("Nurse authenticated successfully.");
        setLoginForm({ username: '', password: '' });
      } else {
        showStatus("Invalid Nurse credentials. Try ID: nurse / Pass: password", "error");
      }
    } else if (role === 'doctor') {
      if (loginForm.username.trim() === 'doctor' && loginForm.password === 'password') {
        setIsLoggedIn(true);
        showStatus("Doctor authenticated successfully.");
        setLoginForm({ username: '', password: '' });
      } else {
        showStatus("Invalid Doctor credentials. Try ID: doctor / Pass: password", "error");
      }
    }
  };
  ...
  const handleRegisterPatient = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPatientForm({ name: '', age: '', gender: 'Male', device_id: '', travel_history: '', past_medical_history: '' });
        await fetchPatients();
  ...
  const getChartData = () => {
    ...
    // Medication indicators as scatter overlay dots
    const medDots = readings.map(r => {
      // Find if a medication was logged within 10 minutes of this reading
      const rTime = new Date(r.timestamp).getTime();
      const hasMed = medications.some(m => {
        const mTime = new Date(m.timestamp).getTime();
        return Math.abs(rTime - mTime) <= 20 * 60 * 1000; // 20 min window
      });
      return hasMed ? r.temperature : null;
    });
      ...
      <div className="login-wrapper">
        <div className="login-card">
          <div className="login-header">
            <div style={{ display: 'inline-flex', width: '55px', height: '55px', background: 'var(--primary-glow)', borderRadius: '16px', alignItems: 'center', justifyContent: 'center', color: '#000', fontWeight: 'bold', fontSize: '1.6rem', marginBottom: '1.25rem', boxShadow: 'var(--glow-shadow)' }}>FS</div>
            <h1 className="login-title">FeverSense AI</h1>
            <p className="login-subtitle">Intelligent Clinical Support for Pyrexia of Unknown Origin</p>
          </div>

          <div className="login-tabs">
            <div 
              className={`login-tab ${role === 'patient' ? 'active' : ''}`}
              onClick={() => { setRole('patient'); setLoginMode('login'); setLoginForm({ username: '', password: '' }); }}
            >
              Patient Home
            </div>
            <div 
              className={`login-tab ${role === 'nurse' ? 'active' : ''}`}
              onClick={() => { setRole('nurse'); setLoginForm({ username: '', password: '' }); }}
            >
              Nurse Station
            </div>
            <div 
              className={`login-tab ${role === 'doctor' ? 'active' : ''}`}
              onClick={() => { setRole('doctor'); setLoginForm({ username: '', password: '' }); }}
            >
              Doctor View
            </div>
          </div>
          ...
          {role === 'patient' && loginMode === 'login' && (
            <form onSubmit={handleLogin}>
              <div className="form-group">
                ...
                <input 
                  type="text" className="form-input" required placeholder="e.g. FS-2026-0012"
              ...
              <div className="form-group">
                ...
                <input 
                  type="password" className="form-input" placeholder="Enter password (optional for demo)"
          ...
          {role === 'patient' && loginMode === 'register' && (
            <form onSubmit={handleRegisterPatient} style={{ maxHeight: '380px', overflowY: 'auto', paddingRight: '0.5rem' }}>
              <div className="form-group">
                ...
                <input 
                  type="text" className="form-input" required placeholder="e.g. John Silva"
              ...
              <div className="form-group">
                ...
                <input 
                  type="number" className="form-input" required placeholder="e.g. 29"
              ...
              <div className="form-group">
                ...
                <textarea 
                  className="form-textarea" placeholder="e.g. Returned from domestic travel to Anuradhapura..."
              ...
              <div className="form-group">
                ...
                <textarea 
                  className="form-textarea" placeholder="e.g. Had dengue fever 3 years ago..."
          ...
          {role === 'nurse' && (
            <form onSubmit={handleLogin}>
              <div className="form-group">
                <label className="form-label">Nurse Staff ID</label>
                <input 
                  type="text" className="form-input" required placeholder="e.g. nurse"
                  value={loginForm.username} onChange={(e) => setLoginForm({ ...loginForm, username: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label className="form-label">Password</label>
                <input 
                  type="password" className="form-input" required placeholder="e.g. password"
                  value={loginForm.password} onChange={(e) => setLoginForm({ ...loginForm, password: e.target.value })}
                />
              </div>
              ...
              <code className="login-help-text">
                💡 Demo Credentials:<br/>
                ID: <code>nurse</code> | Password: <code>password</code>
              </code>
            </form>
          )}
          ...
          {role === 'doctor' && (
            <form onSubmit={handleLogin}>
              <div className="form-group">
                <label className="form-label">Clinical Doctor ID</label>
                <input 
                  type="text" className="form-input" required placeholder="e.g. doctor"
                  value={loginForm.username} onChange={(e) => setLoginForm({ ...loginForm, username: e.target.value })}
                />
              </div>
              <div className="form-group">
              ...
              <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: '1.5rem' }}>
                <LogIn size={18} />
                <span>Authenticate Clinician</span>
              </button>
              <code className="login-help-text">
                💡 Demo Credentials:<br/>
                ID: <code>doctor</code> | Password: <code>password</code>
              </code>
            </form>
          )}
        </div>
    ...
    <div className="app-container">
      ...
      <div className="sidebar">
        <div className="brand">
          ...
          <span className="brand-name">FeverSense AI</span>
        </div>
          ...
          {role === 'patient' && (
            <div>
              <div className="welcome-section">
                <h1 className="welcome-title">FeverSense Home</h1>
