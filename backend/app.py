...
from models import Patient, TemperatureReading, SymptomLog, MedicationLog, SyncRequest
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///fever_sense.db'
...
def create_patient():
    ...
    if not data or not data.get('name') or not data.get('device_id'):
        return jsonify({'error': 'Name and Device ID are required.'}), 400
